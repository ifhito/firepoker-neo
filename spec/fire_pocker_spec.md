...
## 11. UI 要件
- 画面構成
  - ログイン/招待コード入力画面: セッション ID と名前を入力。
  - ダッシュボード: 参加予定セッションと過去セッションを一覧。
  - セッション画面: PBI 詳細パネル、フィボナッチカード、投票状況表示、類似 PBI サイドパネル。
  - 設定画面 (管理者): Notion DB ID、類似 PBI フィルタ条件、ポイント候補設定。
